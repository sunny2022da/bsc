...
func benchInsertChain(b *testing.B, disk bool, gen func(int, *BlockGen), vmCfg vm.Config) {
	...
	chainman, _ := NewBlockChain(db, nil, gspec, nil, ethash.NewFaker(), vmCfg, nil, nil)
	defer chainman.Stop()
	b.ReportAllocs()
	b.ResetTimer()
...
func BenchmarkChainRead_header_10k(b *testing.B) {
	for _, vmCfg := range vmtest.Configs() {
		b.Run(vmtest.Name(vmCfg), func(b *testing.B) {
			benchReadChain(b, false, 10000, vmCfg)
		})
	}
}
func BenchmarkChainRead_full_10k(b *testing.B) {
	for _, vmCfg := range vmtest.Configs() {
		b.Run(vmtest.Name(vmCfg), func(b *testing.B) {
			benchReadChain(b, true, 10000, vmCfg)
		})
	}
}
func BenchmarkChainRead_header_100k(b *testing.B) {
	...
	for _, vmCfg := range vmtest.Configs() {
		b.Run(vmtest.Name(vmCfg), func(b *testing.B) {
			benchReadChain(b, false, 100000, vmCfg)
		})
	}
}
func BenchmarkChainRead_full_100k(b *testing.B) {
	...
	for _, vmCfg := range vmtest.Configs() {
		b.Run(vmtest.Name(vmCfg), func(b *testing.B) {
			benchReadChain(b, true, 100000, vmCfg)
		})
	}
}
func BenchmarkChainRead_header_500k(b *testing.B) {
	...
	for _, vmCfg := range vmtest.Configs() {
		b.Run(vmtest.Name(vmCfg), func(b *testing.B) {
			benchReadChain(b, false, 500000, vmCfg)
		})
	}
}
func BenchmarkChainRead_full_500k(b *testing.B) {
	...
	for _, vmCfg := range vmtest.Configs() {
		b.Run(vmtest.Name(vmCfg), func(b *testing.B) {
			benchReadChain(b, true, 500000, vmCfg)
		})
	}
}
...
func benchReadChain(b *testing.B, full bool, count uint64, vmCfg vm.Config) {
	dir := b.TempDir()
	...
	for i := 0; i < b.N; i++ {
		...
		chain, err := NewBlockChain(db, &cacheConfig, genesis, nil, ethash.NewFaker(), vmCfg, nil, nil)
		if err != nil {
