...
var (
	...
	mirPendingGenerations sync.Map // map[common.Hash]bool - tracks CFGs currently being generated
)
...
func handleMIROptimizationTask(task mirOptimizeTask) {
	switch task.taskType {
	case generateMIR:
		// Check if already cached (another goroutine might have generated it)
		if mirCache.GetCachedCFG(task.hash) != nil {
			// Already cached, remove from pending
			mirPendingGenerations.Delete(task.hash)
			return
		}

		// Check if already being generated by another goroutine
		if _, loaded := mirPendingGenerations.LoadOrStore(task.hash, true); loaded {
			// Another goroutine is already generating this CFG
			return
		}

		// Generate CFG asynchronously
		cfg, err := GenerateMIRCFG(task.hash, task.rawCode)
		if err == nil && cfg != nil {
			mirCache.AddCFGCache(task.hash, cfg)
		}
...
// This function is non-blocking and returns immediately after queuing the task.
func GenOrLoadMIRCFG(hash common.Hash, code []byte) {
	if !enabled || !opcodeParseEnabled {
		return
	}
	// Check if already cached
	if mirCache.GetCachedCFG(hash) != nil {
		return
	}
	// Check if already pending
	if _, pending := mirPendingGenerations.Load(hash); pending {
		return
	}
	// Queue async generation task
	task := mirOptimizeTask{generateMIR, hash, code}
	select {
	case mirTaskChannel <- task:
...
// TryGenerateMIRCFG attempts to generate and cache MIR CFG synchronously (similar to TryGenerateOptimizedCode)
// This is a blocking call that generates CFG immediately. For non-blocking generation, use GenOrLoadMIRCFG.
func TryGenerateMIRCFG(hash common.Hash, code []byte) (*CFG, error) {
	...
	if cachedCFG == nil {
		...
		if err == nil && cachedCFG != nil {
			...
			mirPendingGenerations.Delete(hash)
		}
	}
	return cachedCFG, err
}

// IsMIRCFGPending checks if a CFG is currently being generated asynchronously
func IsMIRCFGPending(hash common.Hash) bool {
	_, pending := mirPendingGenerations.Load(hash)
	return pending
}
