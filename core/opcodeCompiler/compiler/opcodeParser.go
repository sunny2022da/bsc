...
func evalConstFromDef(def *MIR) (uint64, bool) {
	...
	case MirADD, MirSUB:
		if len(def.oprands) < 2 {
			return 0, false
		}
		a, okA := readConstUint(def.oprands[0])
		if !okA {
			if def.oprands[0] != nil && def.oprands[0].kind == Variable && def.oprands[0].def != nil {
				a, okA = evalConstFromDef(def.oprands[0].def)
			}
		}
		b, okB := readConstUint(def.oprands[1])
		if !okB {
			if def.oprands[1] != nil && def.oprands[1].kind == Variable && def.oprands[1].def != nil {
				b, okB = evalConstFromDef(def.oprands[1].def)
			}
		}
		if !okA || !okB {
			return 0, false
	...
	default:
		...
		if len(def.oprands) == 1 && def.oprands[0] != nil {
			if cv, ok := readConstUint(def.oprands[0]); ok {
				return cv, true
			}
			if def.oprands[0].kind == Variable && def.oprands[0].def != nil {
				return evalConstFromDef(def.oprands[0].def)
			}
		}
		return 0, false
	}
