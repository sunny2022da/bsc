			...
			case MirKECCAK256:
				...
				if memoryAccessoraccessor != nil {
					...
					if memData.kind == Konst && len(memData.payload) > 0 {
						...
						// Single-constant identities: if exactly one operand is a constant, apply algebraic identities
						// This reduces runtime work by eliminating trivial ops at build time.
						var konst *uint256.Int
						var other *Value
						leftIsConst := opnd1.kind == Konst
						rightIsConst := opnd2.kind == Konst
						if leftIsConst && !rightIsConst {
							konst = uint256.NewInt(0).SetBytes(opnd1.payload)
							other = opnd2
						} else if rightIsConst && !leftIsConst {
							konst = uint256.NewInt(0).SetBytes(opnd2.payload)
							other = opnd1
						}
						if konst != nil && other != nil {
							switch operation {
							case MirADD:
								if konst.IsZero() {
									// x + 0 = x
							...
							case MirMUL:
								if konst.IsZero() {
									// x * 0 = 0
									newVal := newValue(Konst, nil, nil, []byte{0x00})
									stack.push(newVal)
									optimized = true
								} else if konst.Eq(uint256.NewInt(1)) {
									// x * 1 = x
									stack.push(other)
									optimized = true
								}
							case MirOR:
								if konst.IsZero() {
									...
									stack.push(other)
									optimized = true
								} else if isAllOnes(opnd1.payload) || isAllOnes(opnd2.payload) {
									// x | 0xff..ff = 0xff..ff
									ones := make([]byte, 32)
									for i := range ones {
										ones[i] = 0xff
									}
									stack.push(newValue(Konst, nil, nil, ones))
									optimized = true
								}
							case MirAND:
								if konst.IsZero() {
									...
									// 0xffff..ff (32 bytes all ones)
									allOnes := make([]byte, 32)
									for i := range allOnes {
										allOnes[i] = 0xff
									}
									if len(opnd1.payload) == 32 && leftIsConst && bytesEqual(opnd1.payload, allOnes) {
										stack.push(opnd2)
										optimized = true
									} else if len(opnd2.payload) == 32 && rightIsConst && bytesEqual(opnd2.payload, allOnes) {
										stack.push(opnd1)
										optimized = true
									}
								}
							...
							case MirSHL, MirSHR, MirSAR:
								// Shift amount is the left operand (opnd1)
								if leftIsConst && operation != MirSAR {
									if konst.IsZero() {
										// shift by 0: return value (right operand)
										stack.push(opnd2)
										optimized = true
									} else if konst.Gt(uint256.NewInt(255)) {
										// shift by >=256 yields 0 for logical shifts
										stack.push(newValue(Konst, nil, nil, []byte{0x00}))
										optimized = true
									}
								} else if leftIsConst && operation == MirSAR {
									if konst.IsZero() {
										stack.push(opnd2)
										optimized = true
									} else if konst.Gt(uint256.NewInt(255)) {
										// arithmetic shift by >=256: cannot decide sign at build time unless RHS const
										optimized = false
									}
								}
							case MirEXP:
								// val1 ** val2 ; exponent is right operand
								if rightIsConst {
									if konst.IsZero() {
										// x ** 0 = 1 (including 0**0 per EVM semantics)
										newVal := newValue(Konst, nil, nil, []byte{0x01})
										stack.push(newVal)
										optimized = true
									} else if konst.Eq(uint256.NewInt(1)) {
										// x ** 1 = x
									...
									base := uint256.NewInt(0).SetBytes(opnd1.payload)
									if base.Eq(uint256.NewInt(1)) {
										// 1 ** y = 1
										stack.push(newValue(Konst, nil, nil, []byte{0x01}))
										optimized = true
									}
								}
							}
