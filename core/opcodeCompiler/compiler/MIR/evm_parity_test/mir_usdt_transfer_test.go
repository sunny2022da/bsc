...
func runUSDTTransferTest(t *testing.T, useMIR bool) {
	...
	// NOTE: This test uses the committed USDT *creation* bytecode fixture.
	// (The historical name "usdt.bin" referred to the same artifact.)
	usdtBytecode := loadBytecode(t, "../test_contact/usdt_creation_code.hex")
	...
	if useMIR {
		...
		// MIR is enabled via vm.Config.EnableMIR. No extra compiler hooks are needed in this repo.
	} else {
