	...
	runWithTracer := func(label string, enableMIR bool) ([]callFrame, []jumpiRecord) {
		...
		hooks := &tracing.Hooks{
			...
			OnOpcode: func(pc uint64, op byte, gas, cost uint64, scope tracing.OpContext, rData []byte, depth int, err error) {
				...
				if !enableMIR && (pc == 1075 || pc == 1067 || pc == 2848 || pc == 3236 || pc == 2877 || pc == 3299 || pc == 3373 || pc == 3754 || pc == 3793) {
		...
		if enableMIR {
			...
			// PHI tracing left off in production; re-enable via runner.SetTracePhi(true)
			// from diagnostics code when the PHI resolution path needs inspection.
