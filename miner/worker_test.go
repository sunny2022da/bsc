...
func newTestWorkerBackend(t *testing.T, chainConfig *params.ChainConfig, engine consensus.Engine, db ethdb.Database, n int, vmCfg vm.Config) *testWorkerBackend {
	...
	chain, err := core.NewBlockChain(db, &core.CacheConfig{TrieDirtyDisabled: true}, gspec, nil, engine, vmCfg, nil, nil)
	if err != nil {
...
func testGenerateAndImportBlock(t *testing.T, vmCfg vm.Config) {
	...
	chain, _ := core.NewBlockChain(rawdb.NewMemoryDatabase(), nil, b.genesis, nil, engine, vmCfg, nil, nil)
	defer chain.Stop()
