...
// Function selectors for BUSD contract
var (
...
// Addresses for BUSD contract
var (
	aliceAddr    = common.HexToAddress("0x1000000000000000000000000000000000000001")
	busdContract = common.HexToAddress("0x2000000000000000000000000000000000000001")
	// 全局变量存储实际部署的合约地址
	globalBusdContract common.Address
...
func deployContract(evm *vm.EVM, bytecode []byte) {
	...
	// 更新全局变量存储实际部署的合约地址
	globalBusdContract = contractAddr
	_ = ret // 避免未使用变量警告
}

func mintTokens(evm *vm.EVM, amount *big.Int) {
	// BUSD合约的mint函数签名是 mint(uint256 amount)
	// 不需要to参数，因为BUSD的mint函数会将代币铸造给msg.sender

	// Prepare calldata for BUSD mint function
	...
	executeTransaction(evm, globalBusdContract, calldata, 100000000) // 从10M增加到100M (适合50万次转账)
}
...
func getTokenBalance(evm *vm.EVM, account common.Address) *big.Int {
	...
	// Execute transaction with increased gas limit
	ret := executeTransaction(evm, globalBusdContract, calldata, 100000000) // 从10M增加到100M (适合50万次转账)
...
func performIndividualTransfersWithConfig(evm *vm.EVM, numTransfers int64, gasLimit uint64) time.Duration {
	...
	for i := 0; i < int(numTransfers); i++ {
		...
		// 执行transfer调用
		executeTransaction(evm, globalBusdContract, calldata, gasPerTransfer)
...
func performBatchTransfersWithConfig(evm *vm.EVM, numTransfers int64, gasLimit uint64) time.Duration {
	// BUSD合约没有batchTransferSequential函数，所以这个函数不能用于BUSD
	// 这里保留函数结构，但实际不会被执行
	...
	// Measure execution time
	startTime := time.Now()
	// 不执行任何操作，因为BUSD没有批量转账功能
	duration := time.Since(startTime)

	fmt.Printf("✅ Batch transfer not available for BUSD contract\n")
	log.Printf("✅ Batch transfer not available for BUSD contract")
...
func executeTransaction(evm *vm.EVM, to common.Address, data []byte, gasLimit uint64) []byte {
	...
	//gasUsed := gasLimit - leftOverGas
	//fmt.Printf("✅ Transaction executed, gas used: %d/%d (%.2f%%)\n", gasUsed, gasLimit, float64(gasUsed)/float64(gasLimit)*100)
	return ret
}
